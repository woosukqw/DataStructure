...
void load_names( FILE *fp, int start_year, tNames *names){
	...
	while (1){
		...
		printf("<<bsearch start>> len is %d capa is %d", names->len, names->capacity);
		printf("\n============================\n");
		key = bsearch(&p, names->data, (names->len)+1, sizeof(tName), (int(*)(const void*, const void*)) compare);
		printf("bsearch res:%s", key->name);
		printf("============================\n");
		if (key==NULL){ 
			...
			printf("============================\n");
			printf("%s\n", ((ptr+1)->name) );
			printf("%s %c %d\n", (ptr+idx+1)->name, (ptr+idx+1)->sex,(ptr+idx+1)->freq[0]);
			...
			printf("%s %c %d\n", (ptr+idx+1)->name, (ptr+idx+1)->sex,(ptr+idx+1)->freq[0]);
			printf("%s %c %d\n", (ptr+(names->len))->name, (ptr+(names->len))->sex,(ptr+(names->len))->freq[0]);
			printf("==move후==========================\n");
			for (int i=0; i<names->len+1; i++){
				printf("%s\t%c\t", names->data[i].name, names->data[i].sex); // 이름 성별
			...
			printf("============================\n");
			names->len++;
			printf("idx:%d data[%d].name:%s\n", idx, idx, names->data[idx].name);
			printf("============================\n");
			
		}
		else{
			printf("DUPLICATION!! [%s] [%c] [%d]", key->name, key->sex, year);
...
int compare( const void *n1, const void *n2){
	...
	if (  strcmp( ((tName*)n1)->name,((tName*)n2)->name )==0  ){
		...
		//printf("cmp:%s과%s 비교:%d\n",((tName*)n1)->name,((tName*)n2)->name ,strcmp( ((tName*)n1)->name, ((tName*)n2)->name));
		return strcmp( ((tName*)n1)->name, ((tName*)n2)->name);
		/*
		if ( ((tName*)n1)->name < ((tName*)n2)->name ) return -1;
		else if ( ((tName*)n1)->name > ((tName*)n2)->name ) return 1;
		else return 0;*/
	}
}
